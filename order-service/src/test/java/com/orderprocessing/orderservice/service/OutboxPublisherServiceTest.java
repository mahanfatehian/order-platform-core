...
class OutboxPublisherServiceTest {
    private static final String PAYLOAD =
            "{\"orderId\":\"11111111-1111-1111-1111-111111111111\","
                    + "\"userId\":\"22222222-2222-2222-2222-222222222222\",\"items\":{}}";
    ...
    private OutboxPublisherService publisher(Duration budget) {
        return new OutboxPublisherService(repository, kafkaTemplate, objectMapper, 50, 5, 1000, 60000, budget);
    }
    ...
    void spendsOneTimeoutBudgetAcrossTheBatchRatherThanOnePerEvent() {
        List<OutboxEvent> events = List.of(event("OrderPlacedEvent"), event("OrderPlacedEvent"),
                event("OrderPlacedEvent"), event("OrderPlacedEvent"));
    ...
    void recordsAFailureWithoutSendingWhenThePayloadTypeIsNotOnTheAllowList() {
        List<OutboxEvent> events = List.of(event("SomethingRemovedFromTheRegistry"));
        when(repository.lockReadyBatch(anyInt())).thenReturn(events);
    ...
    void deadLettersAnEventOnceItsAttemptsAreExhausted() {
        OutboxEvent event = event("OrderPlacedEvent");
        event.setAttemptCount(4);
        when(repository.lockReadyBatch(anyInt())).thenReturn(List.of(event));
        when(kafkaTemplate.send(anyString(), anyString(), any())).thenAnswer(invocation -> new StalledSend());

        publisher(Duration.ofMillis(50)).publishUnpublishedEvents();
