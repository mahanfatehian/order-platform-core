...
class EventRetentionBatchServiceIntegrationTest {
    ...
    void deletesOnlyEligibleRowsInExactBatches() {
        ...
        assertThat(count("outbox_events")).isEqualTo(4);
    ...
    private boolean outboxExists(UUID id) {
        return jdbc.queryForObject("select count(*) from outbox_events where id = :id",
    ...
    private long eligibleOutboxCountFromSeparateConnection() {
        try (Connection connection = dataSource.getConnection();
             ...
             ResultSet result = statement.executeQuery(
                     "select count(*) from outbox_events where published = true and published_at < '"
                             + CUTOFF + "'")) {
            result.next();
