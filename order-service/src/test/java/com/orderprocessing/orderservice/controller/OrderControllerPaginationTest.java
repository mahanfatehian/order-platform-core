...
class OrderControllerPaginationTest {
    ...
    void everySortEndsWithTheUniqueTiebreaker() {
        for (String property : List.of("createdAt", "updatedAt", "status")) {
            for (Sort.Direction direction : Sort.Direction.values()) {
                PageRequest request = OrderController.pageRequest(0, 20, property + "," + direction.name().toLowerCase());
    ...
    void anUnknownSortStillCarriesTheTiebreaker() {
        List<Sort.Order> orders = OrderController.pageRequest(0, 20, "notAColumn,desc").getSort().toList();
